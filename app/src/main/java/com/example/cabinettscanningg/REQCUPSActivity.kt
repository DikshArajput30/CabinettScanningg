...
import com.example.cabinettscanningg.databinding.ActivityReqcupsactivityBinding
...
class REQCUPSActivity : AppCompatActivity(),TextToSpeech.OnInitListener,AdapterView.OnItemSelectedListener {
    private lateinit var binding:ActivityReqcupsactivityBinding
    private var context:Context=this@REQCUPSActivity
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding=ActivityReqcupsactivityBinding.inflate(layoutInflater)
        ...
        binding.cabinetscanqc.setOnKeyListener { view, keyCode, keyEvent ->
            ...
            if (keyCode == KeyEvent.KEYCODE_ENTER) {
                ...
                Log.d("smamaktrippsdsd", "onCreate: "+largemrpip)
                if(CabinetName1==" ")
                {
...
//                " INSERT INTO Qccabinet (finyear,srno,date,month,year,username,cabinetname,cabinetitemname,CABINETMOELNO) VALUES (?,?,?,?,?,?,?,?,?)"
            val sql =
                " INSERT INTO REQccabinet (finyear,srno,date,month,year,username,cabinetname,cabinetitemname,CABINETMOELNO) VALUES (?,?,?,?,?,?,?,?,?)"
    ...
    private fun checkduplicate(cabinetsc: String) {
        ...
        try {
            val query ="select cabinetname from REQccabinet where  cabinetname =('${cabinetsc}') "
            val resultSet = statement.executeQuery(query)
            if (resultSet.next())
            {
...
//                    "^XZ\n"
            ...
            Toast.makeText(this, "Please connect to network1", Toast.LENGTH_LONG).show()
        } finally {
...
//////////////////////////on;yy 54
    ...
    private fun truncateTable() {
        try {
            var n = 0
            val sql = "TRUNCATE TABLE RETEMPUPS"
    ...
    private fun insertFollowQC(cabinetname:String,counting:Int) {
        ...
        try {
            ...
            val cstmt = con.prepareCall("{call REUPSCAB  (?,?,?,?,?,?,?,?,?)}")
...
//            }
    ...
    private fun rowcount() {
        try {
            val query = "select count(*)  from REQccabinet"
    ...
    private fun maxvalue() {
        ...
        //  int  maxcount=0;
        try {
            val query = "select MAX(SRNO) As SRNO  from REQccabinet"
            val resultSet: ResultSet = statement.executeQuery(query)
            if (resultSet.next()) {
