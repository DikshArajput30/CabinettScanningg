...
class PartScanning : AppCompatActivity(),TextToSpeech.OnInitListener, AdapterView.OnItemSelectedListener {
    private lateinit var binding: ActivityPartScanningBinding
    private var context:Context=this@PartScanning
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding= ActivityPartScanningBinding.inflate(layoutInflater)
...
//        })
        ...
        binding.transforscaning.setOnKeyListener { view, keyCode, keyEvent ->
            ...
            Log.d("clcikmee", "onCreate: "+"clcikmee")
            if (keyCode == KeyEvent.KEYCODE_ENTER) {
                if (line.equals("Please Select") || line.equals(" ")) {
                    myToastRed(this,"Please Select Lane number")
    ...
    private fun checkpairing(cabinetsc: String) {
        try {
            ...
            {
                binding.pcbscanning.requestFocus()
    ...
    private fun Insertpartpairing() {
        try {
            var n = 0
            val sql =
                " INSERT INTO PAIRING (finyear,srno,date,month,year,cabinetname,pcbname,transformname,username,citemname,cmodelno,pitemname,pmodelno,titemname,tmodelno,cabtype,LINE) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
...
//            statement.setString(4, "bike")
            ...
            statement.setString(12, pcbitemname)
            statement.setString(13, pcbmoelno)
            statement.setString(14, transformitemnme)
            statement.setString(15, transformmodelno)
            statement.setString(16, "UPS")
            statement.setString(17, line)
